...
def match_note_offs(not'''Contains various functions to extract values from midi files or
write values into midi files.'''
...
def read_rhythms(filename, track=0):
...
def write_midifile(filename, sequence, rhythms=None):
